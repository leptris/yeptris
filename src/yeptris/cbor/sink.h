...
typedef struct yep_cbor_sink {
    ...
    int (*text)(void* ctx, const char* s, uint32_t n, uint8_t tag_id, int is_key,
                const char* tag, uint32_t tag_len);
    /* Known integers: value = negative ? -(mag) : mag, mag <= 2^63. */
    int (*int_val)(void* ctx, int negative, uint64_t mag, int is_key,
                   const char* tag, uint32_t tag_len);
...
int yep_cbor_decode_dom_sink(void* d, const unsigned char* p, size_t len, int strict,
                             size_t* consumed);
