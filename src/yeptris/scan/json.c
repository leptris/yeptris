...
/* ---- stage 1: locate structure (the two-stage front) ---------------
 * Per 32-byte chunk: the classifier produces QUOTE/BS/STRUCTURAL/C0
 * bit masks; the resolver walks the merged events in position order —
 * in strings: C0 rejects, backslashes validate the whole escape via
 * the grammar SSOT and jump the walk past it, an UNescaped quote
 * closes; outside: a quote opens (and is a structural), structural
 * bytes emit. Escaped quotes need no parity tracking: every in-string
...
int yep_json_index_build(const char* p, size_t len, uint32_t* structurals, size_t* ns,
    ...
    for (size_t off = 0; off < len; off += 32) {
        ...
        while (events != 0) {
            ...
            if (in_string) {
                if (m.c0 & bit) {
                    *ns = s_count; /* raw C0 inside a string */
                    *nstr = str_count;
                    return 0;
                }
                if (m.bs & bit) {
                    size_t next = 0;
                    if (!yep_json_escape(p, len, at, &next)) {
                        *ns = s_count; /* invalid escape */
                        *nstr = str_count;
                        return 0;
                    }
                    size_t drop = next - off; /* clear events inside the escape */
                    events = drop >= 32 ? 0 : (events & (~0u << drop));
                    continue; /* the escape's own backslash bit is cleared */
                }
                if (m.quote & bit) {
                    /* every in-string backslash is an escape start and
                     * the escape jump above consumed it — including the
                     * \" escape's own quote — so a quote EVENT here is
                     * always a real close (no parity test needed) */
                    in_string = 0;
                    str_close[str_count++] = (uint32_t)at;
                }
                /* in-string structurals: ignored */
            } else {
                if (m.quote & bit) {
                    in_string = 1;
                    structurals[s_count++] = (uint32_t)at;
                } else if (m.structurals & bit) {
                    structurals[s_count++] = (uint32_t)at;
                }
                /* bs/c0 between tokens: stage 2's span rules judge */
            }
            events &= events - 1;
        }
    }
