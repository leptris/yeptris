...
| `~/src/leptris/yeptris-ruby` | yeptris's Ruby binding (own repo, the leptris-ruby pattern) | The Psych-compatible gem: FFI surface, recorder-driven Materializer, Psych drop-in namespace, event-level Handler/Parser; 129 specs |
| `~/src/leptris/yeptris-py` | yeptris's Python binding (own repo) | The PyYAML-compatible package: ctypes FFI (no C extension), one-bulk-drain loader, DOM-mutation dumper, PyYAML constructors ported function-for-function; 55 tests |
