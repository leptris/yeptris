...
Status: CORE LANDED 2026-09-02 — writer JSON flavor (JSON escapes incl \uXXXX, null words, single root; ASAN caught a named-table OOB the unit tests missed), yeptris_serialize_json; jsonc_compat read path (real json-c symbols, -lyeptris-jsonc, enum values pinned); json.hpp nlohmann-flavored C++17 (RAII, move-only, exceptions at the boundary, view children). 2026-09-02 (v2): the BUILDING API landed over DOM mutation (11p3) — json_object_new_{object,array,string,string_len,int,int64,double,boolean} (lazy pending wrappers, O(1) attach, no copies), object_add (replace-on-duplicate returning 1, NULL val deletes), object_del, array_add, array_del_idx, double-attach rejected, standalone-query-then-attach duplicates in and retargets (json-c pointer-lifetime semantics); compact JSON output for to_json_string (json_compact writer flag — public serialize_json format unchanged). 2026-09-02 (v3): PRETTY landed — JSON_C_TO_STRING_PRETTY dispatches to the writer's json_pretty mode (depth-based 2-space indent via a pretty_depth counter — wr_indent is column-based, so pretty tracks depth itself; empty containers stay {} / []; arrays explode per-entry, matching json-c's own pretty layout); array_put_idx landed (yep_mut_seq_set splices the replacement into the entry's position — same in-place shape as map_set; append at idx==len rides attach's retarget contract). 2026-09-03: THE YAJL GENERATOR LANDED — libyeptris-yajl (yajl_compat.{h,c}): yajl_gen_* over the DOM builder with yajl's call-order state machine (maps alternate string-key/value — non-string keys reject with keys_must_be_strings; a dangling key at map_close is the incomplete-pair error; nothing generates after the root closes until reset), gen_get_buf serializes through the compact/pretty JSON writers (no trailing newline), beautify/config accepted, raw gen_number types through the resolver. Pairs attach via the new yeptris_node_map_add_node (pre-built key nodes — the generator's keys are constructed nodes, not caller strings). json-c SUITE BREADTH added (integer/double edges incl. INT64 bounds + exponent truncation, deep nested access, UTF-8 + escape round-trips verbatim, mutation on PARSED documents incl. replace-on-add + array_del_idx ranges). 2026-09-03 (v4): THE YAJL SAX PARSER LANDED — yajl_alloc/free/config/parse/complete_parse/get_error/free_error/get_bytes_consumed/status_to_string over the strict-JSON parse + a DOM walk. Feeds accumulate; every callback fires at complete_parse (batched in time, identical in order/content). Number contract is yajl's: yajl_number carries ALL numbers as text when set; else int64/double discrimination via the resolver's typed accessors, with unrepresentable doubles (1e400) as parse errors. yajl_allow_comments masks JS comments (block + line) with spaces OUTSIDE strings before the strict parse — byte offsets and line numbers preserved, so error positions match yajl's exactly (no YAML superset: the grammar stays RFC 8259). Client cancel from any callback sticks (yajl_status_client_canceled, handle parked); error states stick; completed handles park (yajl's own state model). Trailing-garbage / multiple-values / partial-values config options are REJECTED at config time (zero return, yajl's error contract) rather than silently mis-parsed — extending them means a first-value-extent scan over the json kernel. 16 SAX unit tests (event order, scalar typing incl. quoted "12" = string, number precedence, overflow, cancel, chunked byte-at-a-time feeds, 11 strict rejections, comment masking edges incl. slashes inside strings, invalid UTF-8, deep nesting). REMAINING: none for the compat surface — yajl_tree (yajl_tree) if a consumer needs it, porting yajl's permt tests for the three unsupported flags. 2026-09-02: direct-from-index DOM landed (yep_dom_build_json: JSON entry 63 -> 136 MB/s, 2.2x — see PERF-LEDGER) · Depends: 08, 11, 13, 18 · Layer: `src/yeptris/jsonapi` + `include/yeptris/json*.h` · PLAN.md phase: 6
