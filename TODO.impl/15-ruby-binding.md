...
Status: PHASE A LANDED 2026-09-02 — ffi.rb (every public symbol attached once; check_status/Owned.string seams; pinned enums), Document (sole C-memory owner, Freed-flag double-free guard, GC finalizer, construction conveniences), Node (kind/value/style/tag/anchor accessors, typed reads, seq/map walks, Ruby materialization with alias identity via the node-id memo), Yeptris::YAML load/load_stream/load_file/parse/dump (dump rides the 11p3 mutation API; symbol and date round-trips; cycle refusal), yeptris_node_id added to the C API (query handles are transient — ids are the stable identity bindings key wrapper caches on). 25 specs incl. Psych-verified typing (y/n booleans incl. KEYS, 0o17 vs 017 octal, :sym scanning, timestamp keys). CI: ruby.yml (2 OS). Honest numbers (2k-node doc): dump 1.4× faster than Psych; load 2.7× SLOWER — the DOM-walk materialization pays 2-3 FFI calls per node; phase B's recorder-driven materialization is the designed fix (bulk records, FFI tax per chunk), ≥5× target unchanged. Emitter refinement landed en route: plain-safety now allows a leading/mid colon followed by a non-space (libyaml parity — Psych dumps ":name" plain; 152/152 gates held). 2026-09-02 (phase B): RECORDER-DRIVEN MATERIALIZATION LANDED — Yeptris::Materializer: one bulk drain (records + arena read once), then a pure-Ruby stack machine over the flat unpacked array (per-field FFI reads measured away: 5-8 calls/record -> one unpack). Typing moved to the C resolver SSOT: YeptrisEventRecord's pad byte carries tag_id (sizeof stays 36, ABI-pinned); Ruby converts by tag via Kernel#Integer/Float with three documented Psych-quirk overrides (single-char y/n stay Strings, floats need a dot + signed exponent, sexagesimal via Psych's (e-2).abs formula). yeptris_recorder_new_ex(schema) added (the recorder's engine takes the schema — compat_11 reproduces Psych typing; 47/47 differential cases vs Psych 5.4 verified end-to-end). Merge keys (<<) resolve inline (existing keys win, sequences merge in order). Numbers (2k-node doc): load 2.1x faster than Psych with 2.4x FEWER allocations (28k vs 66k); dump 1.6x. Split profile: C feed 8%, unpack 33%, walk 58% — the 5x target needs C-side bulk conversion or a leaner walk; continues in phase C/D against the 18B harness. 2026-09-02 (suite start): the LOADING-SEMANTICS half of the Psych suite is ported (spec/psych/: boolean complete incl. the Norway problem under both schemas, nil, merge keys incl. bare-hash/deferred inline-map merges and [*a,*b] ordering, alias identity, array/hash/string round-trips, date/time with the YAML 1.1 space-form timestamp normalization) — 42 examples. Fix en route: inline maps under a "<<" key merge at their END, not their start (contents arrive later). 2026-09-02 (phase C start): THE Yeptris::Psych DROP-IN NAMESPACE LANDED — require "yeptris/psych" rebinds the top-level Psych (stdlib Psych kept as Yeptris::Psych::ORIGINAL): load (Psych 5 SAFE-by-default: recursive alias opt-in via AliasNotEnabled, non-core tags raise DisallowedClass — yeptris materializes plain data only, so the class whitelist is structural), unsafe_load, safe_load, dump, parse/parse_stream returning a Psych::Nodes tree that OWNS the yeptris document (children are node handles; GC finalizer releases the C memory; to_ruby reuses the Materializer). 9 compat specs; 77/77 total. 2026-09-03 (phase C+D CORE): OBJECT SERIALIZATION LANDED — C grew node props for synthesized trees (yeptris_node_set_anchor/set_tag/new_alias over arena copies; emit_tag passes SHORTHAND tags verbatim — the !<...> wrapper is for URIs only, fixing double-! output) and Ruby gained Yeptris::Psych::Visitors::{YAMLTree,ToRuby} + CoderShim: dump anchors only SHARED objects (a counting pre-pass — Psych behavior; single-occurrence containers dump clean), !ruby/object via allocate+ivars, encode_with/init_with, !ruby/struct (named + anonymous), !ruby/set, self-referencing structures (allocate-then-fill pre-registration on revival), alias identity. unsafe_load = full revival over the Nodes tree; safe_load/load stay on the Materializer fast path (MECE). READONLY MODE landed (per-node memoized materialization). PLATFORM GEM SCAFFOLDING: rake vendor:local (per-platform lib/platform/<tag>/ copies) + rake audit:symbols (every ffi.rb attach verified against nm T∪U — 48/48). 89/89 binding specs (11 new object-serialization ports incl. shared-identity, self-reference, coder, structs, sets, empty objects, Date/Time-in-objects). LESSONS: node.rb had drifted a SECOND parse_timestamp (DRY — Z-zone timestamps materialized as Date; now Materializer.parse_timestamp is the one implementation, public); Time#iso8601 emits the Z form; dump dispatch must use case/=== (a user == crashed it). 2026-09-03 (phase E): THE EVENT-LEVEL API LANDED — Psych::Handler (all 11 events, no-op base; style constants pin Psych's integers), Psych::Handlers::Recorder ([name, args] pairs), Psych::Parser (one bulk drain via the Materializer's extracted drain seam — parse + records + arena + ONE unpack, then pure-Ruby dispatch; zero per-event FFI). Arg shapes verified DIFFERENTIALLY against stdlib Psych's recorder over 30 cases: all match byte-for-byte (resolved tags, anchors/aliases, block scalars incl. chomping, flow nesting, complex keys, comments, timestamps, bare docs). Three divergences pinned by dedicated specs: %YAML's [major,minor] is not in the record (we pass [], Psych passes [1,1]); synthesized empty scalars normalize ANY->PLAIN + "" at the boundary (a C-side style change broke the emitter round-trip gates — the boundary is the right seam); cross-document aliases error (spec-strict; libyaml's lenience keeps anchors alive across documents — our engine cleared nametabs at DOC_END before this was visible). LESSON: the first 'differential' check was tautological — both sides resolved ::Psych to the REBOUND module; the real differential requires loading stdlib psych/handlers/recorder BEFORE the rebind and reaching through ::Psych::ORIGINAL. 2026-09-03: THE .TML CORPORA PORTED — scripts/port-pure-tml.py converts psych-pure's parse-{block,flow,scalar,props,stream}.tml (68 cases) into the suite frontmatter shape the conformance runner already reads (one converter, no new harness): 67 fixtures checked in at test/conformance/data/psych-pure, wired as the strict conformance-pure ctest — 67/67 on the first run. Two porting notes: the .tml events are flat, the suite tree indents by depth (the converter reconstructs it from the event kinds), and no-final-newline inputs ((<-) marker) are unrepresentable in the suite format — one case skipped, ledgered here. error-suite/parse-suite .tml files only %Import yaml-test-suite (already at 395/395). 2026-09-03 (relocation): THE BINDING MOVED TO ITS OWN REPO — ~/src/leptris/yeptris-ruby (the leptris-ruby pattern: sibling repos). The gem is unchanged (129 specs there, incl. the tag-driven merge fix: a QUOTED '<<' key stays literal — Psych merges on the resolver tag, not the text; found while porting the loader to Python). Its CI checks out leptris/yeptris and runs the suite once published. A PYTHON BINDING exists alongside: ~/src/leptris/yeptris-py (ctypes, no C extension; one bulk drain per parse — the same seam; PyYAML SafeLoader semantics ported from PyYAML's own constructors; 55 tests incl. a PyYAML differential). Both repos are local-only until the user publishes them. REMAINING: publishing/version/lockstep for both bindings + the C gem (USER'S release decisions) · Depends: 11, 12, 13 · Layer: `bindings/ruby` · PLAN.md phase: 3
